...
param serverFarmName string = 'jackyserverfarm-ntier'

@description('Backend name must be unique DNS name worldwide')
param site1_Name string = 'jackybackend-${uniqueString(resourceGroup().id)}'

@description('Frontend name must be unique DNS name worldwide')
param site2_Name string = 'jackyfrontend-${uniqueString(resourceGroup().id)}'
